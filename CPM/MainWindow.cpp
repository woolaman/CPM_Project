...
{
	QString strCMID = ui->lineEdit_CMID->text();
	QString strBKID = ui->lineEdit_BKID->text();
...
void MainWindow::on_pushButton_genPositionLUT_clicked()
{
	...
	{
		QString ofName = "PositionLUT" + QString::number(iDPB);
		QFile ofile(ofName);
		...
		for (size_t iDM = 0; iDM < 2; iDM++)
		{
			...
			{
				QString inFileName = currentPath + "Data/PLUT" + QString::number(DMID) + QString::number(iBK);
...
void MainWindow::on_pushButton_genEnergyLUT_clicked()
{
	for (size_t iDPB = 0; iDPB < 10; iDPB++)
	{
		QString ofName = "EnergyLUT" + QString::number(iDPB);
		QFile ofile(ofName);
		if (!ofile.open(QIODevice::WriteOnly))
		{
		...
		for (size_t iDM = 0; iDM < 2; iDM++)
		{
			...
			for (size_t iBK = 0; iBK < nBK; iBK++)
			{
				QString inFileName = currentPath + "Data/ELUT" + QString::number(DMID) + QString::number(iBK);
				QFile inFile(inFileName);
				if (!inFile.open(QIODevice::ReadOnly))
				{
					qDebug() << "打开文件失败，无法生成能量查找表。";
...
void MainWindow::on_label_floodmap_mouseRightClicked()
{
	if (0 == imgFlag)
	{
		...
		QString figName = "./Data/SegResult_" + strCMID + strBKID + ".png";
...
void MainWindow::on_pushButton_calEnergyResolution_clicked()
{
	...
	figName = "./Data/PeakLoc_" + strCMID + strBKID + ".png";
	image0.save(figName);
	...
	figName = "./Data/ER_Mat_" + strCMID + strBKID + ".png";
	image1.save(figName);
...
void MainWindow::on_pushButton_calUniformity_clicked()
{
	...
	QString figName = "./Data/Uniformity_" + strCMID + strBKID + ".png";
	image.save(figName);
...
void MainWindow::ReStoreData()
{
    QMessageBox::information(this, QString::fromLocal8Bit("读入数据"),
        QString::fromLocal8Bit("开始保存数据到硬盘。"));
    
    const QVector< QVector< QVector<Event> > > &data = dataObject->GetData();
	for (size_t iCM = 0; iCM < nCM; iCM++)
	{
		for (size_t iBK = 0; iBK < nBK; iBK++)
		{
			...
			QString fName = currentPath + "Data/s_" + str_CMID + "_" + QString::number(iBK);
			QFile ofile(fName);
